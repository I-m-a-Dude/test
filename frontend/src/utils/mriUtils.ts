...
// ADAUGĂ ACEASTĂ FUNCȚIE ÎN mriUtils.ts

// Draw slice with RGB overlay data from backend
export const drawSliceWithOverlay = ({
  ...
  const yDim = dims[2];
  const zDim = dims[3];
  // Pentru overlay RGB: backend salvează ca (H, W, D, 3) dar NIfTI header poate să nu reflecte corect
  const channels = 3; // Forțează 3 channels pentru RGB

  if (xDim <= 0 || yDim <= 0 || zDim <= 0) {
    throw new Error('Invalid dimension values');
  }

  console.log(`[OVERLAY] Drawing overlay: ${xDim}x${yDim}x${zDim}, assuming ${channels} RGB channels`);

  let sliceData: Uint8ClampedArray;
  let sliceWidth: number;
  let sliceHeight: number;

  // Convert to appropriate typed array (overlay should be uint8 RGB)
  ...
  switch (header.datatype || header.datatypeCode) {
    case nifti.NIFTI1.TYPE_UINT8:
      ...
      break;
    case nifti.NIFTI1.TYPE_INT16:
      // Convert int16 to uint8 (backend might save as int16)
      ...
      break;
    case nifti.NIFTI1.TYPE_FLOAT32:
      // Convert float32 to uint8 (backend might save as float32)
      const float32Data = new Float32Array(image);
      typedData = new Uint8Array(float32Data.length);
      for (let i = 0; i < float32Data.length; i++) {
        typedData[i] = Math.max(0, Math.min(255, Math.round(float32Data[i])));
      }
      break;
    default:
      console.warn('Unexpected data type for overlay, treating as uint8');
      typedData = new Uint8Array(image);
  }
  ...
  const getVoxelRGB = (x: number, y: number, z: number): [number, number, number] => {
    ...
    // Index calculation for RGB data (H, W, D, C)
    const baseIndex = (z * (xDim * yDim) + y * xDim + x) * channels;

    const r = typedData[baseIndex] || 0;
    const g = typedData[baseIndex + 1] || 0;
    const b = typedData[baseIndex + 2] || 0;
  ...
  try {
    if (axis === 'axial') {
      ...
      for (let j = 0; j < sliceHeight; j++) {
        for (let i = 0; i < sliceWidth; i++) {
          ...
          const finalR = Math.round(avgR / samples);
          const finalG = Math.round(avgG / samples);
          const finalB = Math.round(avgB / samples);

          const index = (j * sliceWidth + i) * 4;
          sliceData[index] = finalR;     // R
          sliceData[index + 1] = finalG; // G
          sliceData[index + 2] = finalB; // B
          sliceData[index + 3] = 255;    // A
        }
      }
    } else if (axis === 'coronal') {
      ...
      for (let j = 0; j < sliceHeight; j++) {
        for (let i = 0; i < sliceWidth; i++) {
          ...
          const index = (j * sliceWidth + i) * 4;
          sliceData[index] = finalR;
          sliceData[index + 1] = finalG;
          sliceData[index + 2] = finalB;
          sliceData[index + 3] = 255;
        }
      }
    } else { // sagittal
    ...
    console.log(`[OVERLAY] Successfully drew ${axis} slice ${currentSlice} (${sliceWidth}x${sliceHeight})`);
