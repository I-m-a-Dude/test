...
export const downloadOverlayResult = async (folderName: string): Promise<File> => {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ detail: 'Eroare la descarcarea overlay-ului' }));
      throw new Error(errorData.detail || `Eroare HTTP: ${response.status}`);
    }
...
function generateAnalysisText(inferenceResult: InferenceResponse, originalPrompt: string): string {
  ...
  return `AUTOMATED MRI ANALYSIS - POST-TREATMENT GLIOMA SEGMENTATION

Dataset analyzed: ${folder_name}
...
📊 General statistics:
• Volume dimensions: ${segmentation_info.shape.join(' × ')} voxels
• Total segmented voxels: ${segmentedVoxels.toLocaleString()} (${segmentationPercentage}% of volume)
• Identified classes: ${foundClasses || 'No pathological tissue detected'}
...
${Object.entries(segmentation_info.class_counts)
  ...
  .map(([cls, count]) => {
    ...
    return `• ${className}: ${count.toLocaleString()} voxels (${percentage}%)`;
  }).join('\n')}

⏱️ Processing performance:
• Preprocessing: ${timing.preprocess_time.toFixed(2)}s
• AI model inference: ${timing.inference_time.toFixed(2)}s  
• Postprocessing: ${timing.postprocess_time.toFixed(2)}s
• **Total: ${timing.total_time.toFixed(2)}s**


⚠️ IMPORTANT NOTE:
This analysis is automatically generated by an artificial intelligence model and is intended for diagnostic assistance. Results MUST be validated by a qualified radiologist before any clinical decision.

🔬 Model used: MedNeXt (MONAI) - BraTS 2024
